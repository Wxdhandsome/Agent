...
def llm_node(state: AgentState) -> AgentState:
    ...
    chat_messages = []
    system_prompt = format_prompt("""你是一个意图判断专家，根据输入判断用户的意图。如果是面试返回'面试'，其他返回'闲聊'""", state, global_vars)
...
def llm_node_2(state: AgentState) -> AgentState:
    ...
    # Build messages from chat history
    chat_messages = []
    system_prompt = format_prompt("""你是一个面试官，请根据用户输入生成一个相关的面试问题""", state, global_vars)
    chat_messages.append(SystemMessage(content=system_prompt))
...
def llm_node_3(state: AgentState) -> AgentState:
    ...
    # Build messages from chat history
    chat_messages = []
    system_prompt = format_prompt("""你是一个友好的助手，请与用户闲聊""", state, global_vars)
    chat_messages.append(SystemMessage(content=system_prompt))
...
def output_node(state: AgentState) -> AgentState:
    ...
    # Output node
    output_value = state.get("final_answer", "")
...
def build_graph():
    ...
    workflow.add_edge("interview_llm", "output_node")  # 面试问题生成(显示) → 最终输出
    workflow.add_edge("chat_llm", "output_node")  # 闲聊模式(显示) → 最终输出
    def _route_condition_node(state: AgentState):
        try:
            if str(state.get("intent_result", "")) == str('面试'):
